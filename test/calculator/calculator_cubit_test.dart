...
void main() {
  ...
  test('deleting the leading operator deletes the whole non-first line',
      () async {
    await cubit.load();
    cubit
      ..inputDigit('1')
  ...
  test('leaving a blank line removes it (only one empty line survives)',
      () async {
    await cubit.load();
    cubit
      ..inputDigit('1')
  ...
  test('the editor never opens a saved sheet — it opens a draft instead',
      () async {
  ...
  test('commit keeps the comment on its line; the new line starts clean',
      () async {
    await cubit.load();
    cubit
      ..inputDigit('1')
  ...
  test('an extra operator (one cap) descends to a new line with it', () async {
    ...
    cubit
      ...
      ..inputOperator('+')
      ..inputDigit('2');
    expect(cubit.state.activeLine!.rawExpression, '1+2'); // one operator: the cap
